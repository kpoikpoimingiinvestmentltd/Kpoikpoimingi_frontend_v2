...
export async function getSystemSettings(): Promise<SystemSettings> {
	const d = (await apiGet(API_ROUTES.settings.getSystemSettings)) as Record<string, unknown>;
	...
	const parseNumber = (v: unknown) => {
		...
		const n = Number(typeof v === "string" ? (v as string).trim() : v);
...
export async function updateVATRate(payload: { vat: number }) {
	...
	const body = payloadTyped?.percentage !== undefined ? { percentage: payloadTyped.percentage } : { percentage: payload.vat };
	return apiPut(API_ROUTES.settings.updateVATRate, body) as Promise<unknown>;
}

export async function updateInterestRate(payload: { interest: number }) {
	// API expects { interestRate: 0.2 }
	const payloadTyped = payload as Record<string, unknown>;
	const body = payloadTyped?.interestRate !== undefined ? { interestRate: payloadTyped.interestRate } : { interestRate: payload.interest };
	return apiPut(API_ROUTES.settings.updateInterestRate, body) as Promise<unknown>;
}
...
export async function updatePenaltyInterestRate(payload: { interest: number }) {
	...
	const payloadTyped = payload as Record<string, unknown>;
	const body = payloadTyped?.interestRate !== undefined ? { interestRate: payloadTyped.interestRate } : { interestRate: payload.interest };
	return apiPut(API_ROUTES.settings.updatePenaltyInterestRate, body) as Promise<unknown>;
}

export function useUpdateVAT() {
	const qc = useQueryClient();
	return useMutation({
		mutationFn: (p: { vat?: number; percentage?: number }) => updateVATRate(p as { vat: number }),
		onSuccess: () => {
...
export function useUpdateInterest() {
	const qc = useQueryClient();
	return useMutation({
		mutationFn: (p: { interest?: number; interestRate?: number }) => updateInterestRate(p as { interest: number }),
...
export function useUpdatePenaltyInterest() {
	const qc = useQueryClient();
	return useMutation({
		mutationFn: (p: { interest?: number; interestRate?: number }) => updatePenaltyInterestRate(p as { interest: number }),
		onSuccess: () => qc.invalidateQueries({ queryKey: ["systemSettings"] }),
	});
}
