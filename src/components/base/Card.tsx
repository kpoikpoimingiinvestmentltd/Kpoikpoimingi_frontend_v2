...
export type Payment = { id: string; status?: "Successful" | "Failed"; date: string };

/**
 * Card - small payment/receipt card used across the app.
 * Inputs: p (Payment), onView callback, variant and optional icon.
 * Output: rendered card with status/date and a View action.
 */
	...
	return (
		<div className="bg-card py-6 rounded-lg border border-dashed border-gray-200 p-3 flex flex-col gap-y-2 items-center justify-center">
			{variant === "icon" ? (
				...
				<>
					<StatusBadge status={(p.status ?? "Successful") as string} />
...
export function CardIcon(props: { p: Payment; onView: (p: Payment) => void; icon?: React.ReactNode }) {
	const { p, onView, icon } = props;
	return <Card p={p} onView={onView} variant="icon" icon={icon} />;
}
