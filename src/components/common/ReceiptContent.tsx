...
export default function ReceiptContent({ receipt }: ReceiptContentProps) {
  const isFullPayment =
    !receipt.totalInstallments || receipt.totalInstallments === 1;
  ...
  const installmentParsed = parseInstallmentProgress(
    receipt.installmentProgress
  );
  const remainingBalanceRaw =
    receipt.contract && (receipt.contract as any).outStandingBalance != null
      ? (receipt.contract as any).outStandingBalance
      : null;
      ...
      <>
        ...
        <main className="flex-grow">
          <section className="flex flex-col gap-y-1 mb-6 mt-4">
            <h3 className="text-center text-lg font-medium text-primary mb-4">
              Invoice With Full Payment
            </h3>
            <KeyValueRow
              label="Name"
              value={receipt.customer?.fullName ?? receipt.id ?? "-"}
            />
            <KeyValueRow label="Email" value={receipt.customer?.email ?? "-"} />
            <KeyValueRow
              label="Whatsapp number"
              value={receipt.customer?.phoneNumber ?? "-"}
            />
            <KeyValueRow
              label="Home Address"
          ...
          <section className="mt-4 flex flex-col gap-y-4">
            ...
            <CustomCard className="grid grid-cols-1 gap-y-1 px-4 py-5 bg-card border-0 dark:border">
              <KeyValueRow
                label="Property Name"
                value={
                  receipt.contract?.property?.name ??
              ...
              <KeyValueRow
                ...
                value={
                  receipt.vatAmount
                    ? `₦${Number(receipt.vatAmount).toLocaleString()}`
                    : "-"
                }
                leftClassName="text-gray-600"
                rightClassName="text-right font-medium"
              ...
              <KeyValueRow
                ...
                value={
                  receipt.amountPaid && receipt.vatAmount
                    ? `₦${(
                        Number(receipt.amountPaid) + Number(receipt.vatAmount)
                      ).toLocaleString()}`
                    : "-"
                }
                leftClassName="text-gray-600"
          ...
          <section className="md:w-11/12 mx-auto mt-8 font-normal text-center py-4">
            <p className="text-sm sm:text-[.9rem] dark:text-gray-200 text-gray-700">
    ...
    <>
      ...
      <main className="flex-grow">
        <section className="flex flex-col gap-y-3 mb-4 mt-3">
          <h3 className="text-center text-sm sm:text-lg font-medium text-primary ">
        ...
        <section className="mt-4 flex flex-col gap-y-4">
          <header className="flex items-center justify-between gap-1 text-gray-800 dark:text-gray-100 bg-primary/10 dark:bg-primary/50 px-4 md:px-6 py-2.5 rounded-md">
            ...
            {!isMigrated && (
              <span className="text-xs sm:text-sm font-medium text-center">
                Payment duration (
                {receipt.contract?.durationValue ??
                  receipt.totalInstallments ??
                  "-"}{" "}
                {receipt.contract?.durationUnit?.duration?.toLowerCase() ===
                "weeks"
                  ? "weeks"
                  : "months"}
                )
          ...
          <CustomCard className="grid grid-cols-1 gap-y-1 px-4 py-5 bg-card border-0 dark:border">
            <KeyValueRow
              label="Property Name"
              value={
                receipt.contract?.property?.name ?? receipt.propertyName ?? "-"
              }
              leftClassName="text-gray-600"
              rightClassName="text-right font-medium"
            />
            <KeyValueRow
              label="Total amount"
              value={
                receipt.contract?.property?.price
                  ? `₦${Number(
                      receipt.contract.property.price
                    ).toLocaleString()}`
                  : "-"
            ...
            <KeyValueRow
              ...
              value={
                receipt.contract?.downPayment
                  ? `₦${Number(receipt.contract.downPayment).toLocaleString()}`
                  : "-"
              }
              leftClassName="text-gray-600"
              rightClassName="text-right font-medium"
            ...
            <KeyValueRow
              ...
              value={
                remainingBalanceRaw != null
                  ? `₦${Number(remainingBalanceRaw).toLocaleString()}`
                  : "-"
              }
              leftClassName="text-gray-600"
              rightClassName="text-right font-medium"
        ...
        {!isMigrated ? (
          <section className="md:w-11/12 mx-auto mt-4 text-center py-4 px-4">
            {installmentParsed.covered === installmentParsed.total &&
            installmentParsed.total > 0 ? (
              <p className="text-xs font-normal sm:text-[.9rem] dark:text-gray-100 text-gray-700">
                Thank you for completing all your installment payments. We
                appreciate your timely payment and look forward to serving you
                again in the future.
              </p>
            ) : (
              <p className="text-xs font-normal sm:text-[.9rem] dark:text-gray-100 text-gray-700">
                ...
                <span className="font-semibold">
                  {receipt.nextPaymentDate
                    ? new Date(receipt.nextPaymentDate).toLocaleDateString(
                        "en-GB"
                      )
                    : "-"}
                </span>
                . Endeavour to make payment. Failure to make payment attracts an
