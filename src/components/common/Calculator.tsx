...
export default function Calculator() {
  ...
  // Duration = number of periods (e.g. 78 weeks, 24 months). No artificial cap; use a high max for UX.
  const maxDuration = 999;

  const handleDurationChange = (value: number) => {
    const n = Number(value);
    if (n >= 0 && n <= maxDuration) setDuration(n);
  };
      ...
      >
        <div
          className={`fixed z-50 right-6 bottom-6 before:content-[''] before:inset-0 before:fixed ${
            open
              ? "before:z-[29] before:bg-black/50 before:pointer-events-auto"
          ...
          <PopoverContent className="w-80 mr-10 mb-8 sm:px-8 sm:pb-5 sm:mr-24 sm:mb-0 relative p-4">
            ...
            <div className="relative">
              ...
              <div className="flex flex-col gap-y-2.5 relative">
                ...
                />
                <div className="">
                  <Label className={labelStyle("font-normal mb-1")}>
                    Interval
                  </Label>
                  <Select
                    value={interval}
                  ...
                  >
                    <SelectTrigger
                      className={twMerge(
                        "w-full text-sm min-h-9 rounded-sm border",
                      )}
                    >
                      <SelectValue />
                    </SelectTrigger>
