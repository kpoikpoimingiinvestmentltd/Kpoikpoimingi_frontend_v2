  ...
  useEffect(() => {
    ...
    const avatarUrl =
      Array.isArray(user?.media) && (user?.media as unknown[])?.length > 0
        ? (((user?.media as unknown[])[0] as Record<string, unknown>)
            ?.fileUrl as string)
    ...
    // Update user status
    const status =
      ((user?.status as Record<string, unknown>)?.status as string) ||
      (user?.statusId as string);
    ...
    suspendMutation.mutate(userId, {
      onSuccess: (res: SuspendUserResponse) => {
        try {
          ...
          const statusObj = res.user?.status as unknown as
  ...
  return (
    <PageWrapper>
      <div className="flex items-center justify-between flex-wrap gap-4 mb-4">
        <PageTitles
          title="User Profile"
          description="User profile contains the details of this user"
        />
        <div className="flex items-center gap-4 flex-wrap">
          <button
            onClick={openEdit}
            className={
              "flex items-center text-sm sm:text-base underline-offset-2 underline"
            }
          >
            ...
            <div className="absolute bottom-0 translate-y-1/2 left-4 sm:left-10">
              <Avatar className="size-32 bg-white">
                <AvatarImage
                  ...
                  src={
                    Array.isArray(
                      (currentUser as Record<string, unknown>)?.media
                    ) &&
                    (
          ...
          <div className="mt-24 grid grid-cols-1 gap-4">
              ...
              {(() => {
                ...
                const name =
                  ((currentUser as Record<string, unknown>)
                    ?.username as string) ?? "-";
                const email =
                  ((currentUser as Record<string, unknown>)?.email as string) ??
                  "-";
                const phone =
                  ((currentUser as Record<string, unknown>)
                ...
                const stateObj = (currentUser as Record<string, unknown>)
                  ?.stateOfOrigin;
                const state =
                ...
                const dobRaw =
                  ...
                  null;
                const dob = dobRaw
                  ? new Date(dobRaw).toLocaleDateString()
                  : "-";
                const roleLabel =
                  typeof (currentUser as Record<string, unknown>)?.role ===
                  "string"
                ...
                const assigned =
                  ...
                  ((currentUser as Record<string, unknown>)
                    ?.assignedCustomersCount as number) ??
                  ((currentUser as Record<string, unknown>)
                    ?.assignedCount as number) ??
                  0;
                return (
                  <>
                    ...
                    <KeyValueRow label="Assigned Customers">
                      <Badge
                        ...
                        status="primary"
                        label={<span>{assigned} Assigned</span>}
                        size="md"
                      />
                    </KeyValueRow>
                  </>
            ...
            <div className="space-y-4">
              {(() => {
                ...
                const accountTypeObj = (currentUser as Record<string, unknown>)
                  ?.accountType;
                const accountType =
                  typeof accountTypeObj === "string"
                    ? accountTypeObj
                    : ((accountTypeObj as Record<string, unknown>)
                        ?.type as string) ?? "-";

                // Extract bank name - handle both string and object
                const bankNameObj = (currentUser as Record<string, unknown>)
                  ?.bankName;
                const bankName =
                  typeof bankNameObj === "string"
                    ? bankNameObj
                    : ((bankNameObj as Record<string, unknown>)
                        ?.name as string) ?? "-";
