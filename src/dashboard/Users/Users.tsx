...
export default function Users() {
  ...
  const [deactivateConfirmOpen, setDeactivateConfirmOpen] =
  ...
  const [resetPasswordConfirmOpen, setResetPasswordConfirmOpen] =
    React.useState(false);
  const [resetPasswordSuccessOpen, setResetPasswordSuccessOpen] =
    React.useState(false);
  ...
  const [avatarMediaKey, setAvatarMediaKey] = React.useState<string | null>(
    null
  );
  ...
  const paginationData = (
    usersData as { pagination?: { total?: number; totalPages?: number } }
  )?.pagination;
  ...
  React.useEffect(() => {
    ...
    const stateOfOriginId =
      typeof user?.stateOfOrigin === "object"
        ? (user?.stateOfOrigin as Record<string, unknown>)?.id
        : user?.stateOfOrigin;
    const roleId =
      typeof user?.role === "object"
        ? (user?.role as Record<string, unknown>)?.id
        : user?.role;
    const accountTypeId =
      typeof user?.accountType === "object"
        ? (user?.accountType as Record<string, unknown>)?.id
        : user?.accountTypeId || user?.accountType;
    const bankNameId =
      typeof user?.bankName === "object"
        ? (user?.bankName as Record<string, unknown>)?.id
        : user?.bankName;

    let avatarUrl: string | null = null;
    if (Array.isArray(user?.media) && (user?.media as unknown[])?.length > 0) {
      const mediaItem = (user?.media as unknown[])[0] as Record<
        string,
        unknown
      >;
      avatarUrl =
        (mediaItem?.fileUrl as string) || (mediaItem?.url as string) || null;
    ...
    const dobRaw =
      (user?.dateOfBirth as string) ?? (user?.dob as string) ?? null;
    const dobFormatted = dobRaw
      ? new Date(dobRaw).toISOString().split("T")[0]
      : "";
    ...
    if (typeof val === "object") {
      ...
      if (obj.role)
        return typeof obj.role === "string"
          ? obj.role
          : String(
              (obj.role as Record<string, unknown>).name ??
                JSON.stringify(obj.role)
            );
  ...
  const getName = (r: unknown) =>
    isUser(r)
      ? r.fullName ?? r.username ?? r.email ?? r.id
      : (r as Record<string, unknown>).name ?? "-";
  const getPhone = (r: unknown) =>
    isUser(r)
      ? r.phoneNumber ?? "-"
  ...
  const getAssigned = (r: unknown) =>
    isUser(r)
      ? r.numberOfAssignedCustomers ?? "-"
      : (r as Record<string, unknown>).numberOfAssignedCustomers ?? "-";
  const getSalary = (r: unknown) =>
    (r as Record<string, unknown>).salaryAmount ?? "-";

  return (
    <div className="flex flex-col gap-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4 mb-4">
        <PageTitles
          title="Employed Users"
          description="This are the list of all the employees / users working for Kpoi kpoi mingi investment"
        />
                              ...
                              >
                                {[
                                  ...
                                  },
                                  {
                                    ...
                                    action: () => {
                                      setToDeactivate({
                                        id: (row as Record<string, unknown>)
                                          .id as string,
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className={modalContentStyle()}>
          ...
          <UserForm
            ...
            submitLabel={
              updateMutation.isPending || uploadProfileMutation.isPending
                ? "Saving..."
                : "Save Changes"
            }
            isLoading={
      ...
      <ConfirmModal
        ...
        actions={[
          { label: "Cancel", onClick: () => true, variant: "ghost" },
          {
            label: suspendUserMutation.isPending
              ? "Deactivating..."
              : "Deactivate",
            onClick: async () => {
      ...
      <ConfirmModal
        ...
        actions={[
          { label: "Cancel", onClick: () => true, variant: "ghost" },
          {
            label: resetPasswordMutation.isPending
              ? "Resetting..."
              : "Reset Password",
            onClick: async () => {
