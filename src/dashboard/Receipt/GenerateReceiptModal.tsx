...
export default function GenerateReceiptModal({ open, onOpenChange }: { open: boolean; onOpenChange: (o: boolean) => void }) {
	...
	const handleFormSubmit = (data: GenerateReceiptFormData) => {
		...
		if (!data.generatedBy || data.generatedBy.trim() === "") {
			toast.error("Please enter who generated the receipt");
			return;
		}
					...
					<form onSubmit={handleSubmit(handleFormSubmit)} className="">
						<div className="grid max-w-lg grid-cols-1 md:grid-cols-2 gap-4 mx-auto md:gap-x-8 md:gap-y-5 py-4 mt-4">
							...
							<Controller
								name="generatedBy"
								control={control}
								rules={{ required: "Generated by is required" }}
								render={({ field }) => (
									<CustomInput label="Generated By" containerClassName="col-span-2" required {...field} className={inputStyle} />
								)}
							/>
