...
export default function GenerateReceiptModal({
  ...
  const [generatedReceipt, setGeneratedReceipt] =
    React.useState<ReceiptDetail | null>(null);

  const { control, handleSubmit, watch, setValue, reset } =
    useForm<GenerateReceiptFormData>({
      defaultValues: {
        customerId: "",
        paymentMethodId: "",
        amountPaid: "",
  ...
  const { data: activeContractCustomers, isLoading: activeContractLoading } =
    useGetCustomersWithActiveContract(
      currentPage,
      10,
      customerSearch,
      "name",
  ...
  React.useEffect(() => {
    ...
    const selectedCustomer = activeContractCustomers.data.find(
  ...
  return (
    <>
                      ...
                      >
                        ...
                        >
                          {activeContractLoading ? (
                            ...
                            <>
                              {activeContractCustomers?.data?.map(
                                (customer: any) => (
                                  <DropdownMenuItem
                                    key={customer.id}
      ...
      <ReceiptDisplayModal
        open={showReceiptDisplay}
        onOpenChange={setShowReceiptDisplay}
