...
export default function PropertyDetailsSection({
  ...
  const properties: PropertyData[] = React.useMemo(() => {
    ...
    if (Array.isArray(dataObj.data as unknown))
      return dataObj.data as unknown as PropertyData[];
    if (Array.isArray(dataObj.items as unknown))
      return dataObj.items as unknown as PropertyData[];
    return [];
  }, [propertiesData]);

  React.useEffect(() => {
    if (form.paymentFrequency) {
      const selectedFrequency = paymentFrequencyOptions.find(
        (o) => o.key === form.paymentFrequency,
      );
    ...
    <div className={centeredContainer()}>
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          >
            <SelectTrigger
              className={twMerge(inputStyle, "w-full min-h-11 cursor-pointer")}
            >
      ...
      {form.isCustomProperty && (
        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <CustomInput
            ...
            onChange={(e) =>
              handleChange("customPropertyPrice", e.target.value)
            }
            labelClassName={labelStyle()}
            className={twMerge(inputStyle)}
          />
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <Select
            value={form.paymentDurationUnit}
            onValueChange={(v) => handleChange("paymentDurationUnit", v)}
          >
            <SelectTrigger
              className={twMerge(inputStyle, "w-full min-h-11 cursor-pointer")}
              disabled
          ...
          <p className="text-xs text-stone-500 mt-1">
            {form.paymentFrequency &&
              "Automatically synced with payment interval"}
          </p>
        </div>
        ...
        <div>
          ...
          <Select
            ...
            onValueChange={(v) => handleChange("paymentDuration", v)}
          >
            <SelectTrigger
              className={twMerge(inputStyle, "w-full min-h-11 cursor-pointer")}
            >
      ...
      <ValidationErrorDisplay
        ...
        filter={(field) =>
          ["Property name", "Payment duration"].some((keyword) =>
