...
export default function Customers() {
  ...
  const handleExportFiltered = async () => {
    try {
      ...
      const link = document.createElement("a");
      link.href = url;
      link.download = `customers-${debouncedSearch}-${new Date()
        .toISOString()
        .slice(0, 10)}.csv`;
      document.body.appendChild(link);
      link.click();
  ...
  const pages = React.useMemo(() => {
    ...
    const total =
      (cd.pagination && typeof cd.pagination.total === "number"
        ? cd.pagination.total
        : 0) || 0;
  ...
  return (
    <div className="flex flex-col gap-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4 mb-4">
        <PageTitles
          title="Customers"
          description="List of people who patronize Kpoikpoimingi investment"
        />
                      ...
                      {customersList.map((row, idx: number) => (
                        ...
                        >
                          <TableCell title={row.customerCode}>
                            <span className="max-w-40 block truncate">
                              {row.customerCode}
                            </span>
                          </TableCell>
                          ...
                          <TableCell className="flex items-center justify-end gap-1">
                            {row.registrations?.some(
                              ...
                              <Link
                                to={`${_router.dashboard.customerDetails.replace(
                                  ":id",
                                  row.id
                                )}?tab=details`}
