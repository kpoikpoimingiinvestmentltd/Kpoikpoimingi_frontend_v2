...
export default function TabCustomerDetails({
  ...
  const getEmploymentStatusLabel = (
    status?: string | number | null | { status?: string },
  ): string => {
    if (status === null || status === undefined) return "N/A";

    // Handle numeric ID format (employmentStatusId from API)
    if (typeof status === "number") {
      const option = employmentStatusOptions.find(
        (opt) => opt.key === String(status),
      );
  ...
  allRegistrations.forEach((r) => {
    ...
    candidates.forEach((p) => {
      if (p && typeof p === "object")
        allProps.push(p as Record<string, unknown>);
    });
  });
  ...
  const propertyNames = allProps
    .map((p) => {
      ...
      if (
        typeof p["customPropertyName"] === "string" &&
        p["customPropertyName"]
      )
        return p["customPropertyName"] as string;
  ...
  const totalQuantity =
    allProps.reduce((acc, p) => acc + (Number(p.quantity) || 0), 0) ||
  ...
  const totalAmount = allProps.reduce((acc, p) => {
    ...
    const price =
      Number(
        p.customPropertyPrice ??
          p.customPrice ??
          nestedProperty?.price ??
          p.price ??
          p.downPayment ??
          0,
      ) || 0;
  ...
  const pt =
    typeof rawPt === "number"
      ? rawPt
      : typeof rawPt === "string" && /^\d+$/.test(rawPt)
        ? Number(rawPt)
        : undefined;
  ...
  const paymentMethodLabel =
    pt === 2 || String(pt) === "2" || allPropsAreCustom
      ? "One time payment"
      : "Hire Purchase";
      ...
      <CustomCard className="mt-6 grid grid-cols-1 gap-6 md:p-8 bg-card">
        ...
        <section className={titleSectionStyle}>
          ...
          <div>
            <KeyValueRow
              ...
              label="NIN"
              variant="files"
              files={transformMediaFiles(
                registration?.mediaFiles?.identificationDocument ?? [],
              )}
              leftClassName="text-sm text-muted-foreground"
            />
            <KeyValueRow
              ...
              variant="files"
              files={transformMediaFiles(
                registration?.mediaFiles?.driverLicense ?? [],
              )}
              leftClassName="text-sm text-muted-foreground"
            />
            <KeyValueRow
              ...
              variant="files"
              files={transformMediaFiles(
                registration?.mediaFiles?.indegeneCertificate ?? [],
              )}
              leftClassName="text-sm text-muted-foreground"
            />
            <KeyValueRow
              ...
              variant="files"
              files={transformMediaFiles(
                registration?.mediaFiles?.signedContract ?? [],
              )}
              leftClassName="text-sm text-muted-foreground"
            />
          </div>
        ...
        <div>
          <small className="text-[#131212B2] dark:text-white">
            I hereby authorise{" "}
            <b className="font-medium dark:text-primary text-black">
              Kpoi Kpoi Mingi Investments Ltd
            </b>{" "}
            to retrieve the electrical appliance from me, or any other person at
            my or any other place it may be found in the event of my default in
            paying the Hire Purchase sum as agreed.
          </small>
        </div>
        {!isFullPayment && (
          <section className={titleSectionStyle}>
              ...
              children={
                <>
                  <small className="text-[#131212B2] dark:text-gray-100">
                    As a guarantor, I hereby guaranty to pay all sums due under
                    the Hire Purchase Agreement in the event of default by the
                    Applicant. <br />
                    <br /> I accept that messages, notices, processes and other
                    correspondences where necessary, sent to my WhatsApp number
                    as shown herein are properly delivered and served on me.
                  </small>
                </>
              }
            />
            <div className="grid grid-cols-1 gap-2">
              ...
              <KeyValueRow
                label="Employment status"
                value={getEmploymentStatusLabel(
                  guarantors[0]?.employmentStatusId ??
                    guarantors[0]?.employmentStatus,
                )}
                leftClassName="text-sm text-muted-foreground"
                rightClassName="text-right"
              />
              ...
              <KeyValueRow
                label="Business address"
                value={
                  guarantors[0]?.companyAddress ||
                  guarantors[0]?.businessAddress ||
                  "N/A"
                }
                leftClassName="text-sm text-muted-foreground"
                rightClassName="text-right"
              ...
              <KeyValueRow
                ...
                variant="files"
                files={transformMediaFiles(
                  registration?.mediaFiles?.guarantor_0_doc ?? [],
                )}
                leftClassName="text-sm text-muted-foreground"
              />
            </div>
          ...
          <section className={titleSectionStyle}>
            ...
            <div className="grid grid-cols-1 gap-2">
              ...
              <KeyValueRow
                label="Business address"
                value={
                  guarantors[1]?.businessAddress ||
                  guarantors[1]?.companyAddress ||
                  "N/A"
                }
                leftClassName="text-sm text-muted-foreground"
                rightClassName="text-right"
              ...
              <KeyValueRow
                label="Employment status"
                value={getEmploymentStatusLabel(
                  guarantors[1]?.employmentStatusId ??
                    guarantors[1]?.employmentStatus,
                )}
                leftClassName="text-sm text-muted-foreground"
                rightClassName="text-right"
              />
              <KeyValueRow
                ...
                variant="files"
                files={transformMediaFiles(
                  registration?.mediaFiles?.guarantor_1_doc ?? [],
                )}
                leftClassName="text-sm text-muted-foreground"
              />
            </div>
