...
export default function AddCustomer() {
  ...
  const statePaymentMethod = (location.state as LocationState | undefined)
    ?.paymentMethod;
  ...
  if (querySelectedProperties) {
    try {
      selectedProperties = JSON.parse(
        decodeURIComponent(querySelectedProperties)
      );
    } catch {
      selectedProperties = undefined;
    }
  } else {
    selectedProperties = (location.state as LocationState | undefined)
      ?.selectedProperties;
  }
  ...
  return (
    <PageWrapper>
      <div className="flex items-center justify-between flex-wrap gap-4 mb-4">
        <PageTitles
          title="Add Customers"
          description="Fill in the details to add customer to Kpoikpoimingi investment"
        />
      </div>
      ...
      <CustomCard>
        <div className="p-6">
          <CustomerForm
            paymentMethod={
              paymentMethod === "once" || paymentMethod === "installment"
                ? (paymentMethod as "once" | "installment")
                : undefined
            }
